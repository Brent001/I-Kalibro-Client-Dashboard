<script lang="ts">
  ...
  let loading = false;
  let error = "";
  let selectedJournal: Book | null = null;
  let actionLoading = false;
  let cancellingBookId: number | null = null;
  let reservedJournalIds: number[] = [];
  let borrowedJournalIds: number[] = [];

  $: journals.forEach(book => {
  ...
  async function fetchUserBookStatus() {
    try {
      const res = await apiCall(`/api/books/transaction?userId=${currentUser.id}&itemType=${ITEM_TYPE}`);
      reservedJournalIds = res.reservedBookIds || [];
      borrowedJournalIds = res.borrowedBookIds || [];
      if (Array.isArray(reservedJournalIds) && Array.isArray(borrowedJournalIds) && borrowedJournalIds.length > 0) {
        reservedJournalIds = reservedJournalIds.filter(id => !borrowedJournalIds.includes(id));
      }
    } catch { reservedJournalIds = []; borrowedJournalIds = []; }
  }
  ...
  async function fetchBooks(page = 1) {
    ...
    try {
      ...
      const result = await apiCall(`/api/journals?${params.toString()}`);
      journals = result.data.journals;
      totalPages = result.data.pagination.totalPages || 1;
      totalJournals = result.data.pagination.totalItems || 0;
      await fetchUserBookStatus();
    } catch (err) {
      error = err instanceof Error ? err.message : 'Failed to fetch journals';
      journals = [];
  ...
  async function handleBookAction(book: Book) {
    ...
    if (reservedJournalIds.includes(book.id) || borrowedJournalIds.includes(book.id)) return;
    ...
    try {
      ...
      await apiCall('/api/books/transaction', 'POST', {
        itemId: Number(book.id), itemType: ITEM_TYPE, userId: Number(currentUser.id),
        requestType: 'reserve', requestedBorrowDate, requestedDueDate: due.toISOString().split('T')[0]
      });
      selectedJournal = null;
      await fetchBooks(currentPage);
    } catch (err) {
      error = err instanceof Error ? err.message : 'Failed to reserve journal';
  ...
  function openBookModal(book: Book) { selectedJournal = book; }
  ...
  async function fetchCategories() {
    try {
      const res = await fetch(`/api/books/categories?itemType=${ITEM_TYPE}`, { credentials: 'include' });
...
<div class="w-full space-y-2 text-sm" style="color: #2C1A0E;">
  ...
  <div class="relative overflow-hidden rounded-xl shadow-sm px-3 py-3 sm:px-5 sm:py-3.5 border"
    ...
    <div class="relative z-10 flex items-center gap-3">
      ...
      <div class="flex-1 min-w-0">
        <h1 class="text-lg sm:text-xl font-bold leading-tight" style="color: #F5F0E8;">Journal Catalog</h1>
        <p class="text-xs sm:text-sm mt-0.5 hidden sm:block" style="color: rgba(245,240,232,0.7);">Browse and reserve journals from our collection</p>
      </div>
      <!-- Desktop view toggle -->
  ...
  <div class="rounded-xl shadow-sm p-2 sm:p-4 border" style="background: #F5F0E8; border-color: #D4C4A8;">
    <form on:submit|preventDefault={performSearch} class="flex gap-2 items-center">
      <div class="relative flex-1 min-w-0">
        ...
        <input type="text" placeholder="Search title, author…" bind:value={searchInputValue} disabled={loading} autocomplete="off"
          ...
          style="background: #FDF8F0; border: 1.5px solid #D4C4A8; color: #2C1A0E;"
    ...
    <div class="flex items-center justify-between mt-2 pt-2 gap-2" style="border-top: 1px solid #D4C4A8;">
      <p class="text-sm flex items-center gap-2 flex-wrap min-w-0" style="color: #7A5A2A;">
        ...
        {:else}
          <strong style="color: #2C1A0E;">{totalJournals.toLocaleString()}</strong> journals
          {#if searchTerm}
  ...
  {:else if journals.length === 0}
    <div class="rounded-xl border text-center py-12 px-3" style="background: #F5F0E8; border-color: #D4C4A8;">
      ...
      <h3 class="text-base font-bold mb-1.5" style="color: #2C1A0E;">No journals found</h3>
          ...
          <table class="w-full border-collapse min-w-[540px]">
            <thead>
              <tr style="background: #EDE4D4; border-bottom: 1.5px solid #D4C4A8;">
                <th class="px-3 sm:px-5 py-3.5 text-left text-xs font-bold uppercase tracking-widest" style="color: #7A5A2A;">Journal</th>
...
{#if selectedJournal}
  <JournalModal journal={selectedJournal} reservedJournalIds={reservedJournalIds} borrowedJournalIds={borrowedJournalIds} {actionLoading} itemType={ITEM_TYPE}
    onClose={() => selectedJournal = null} onReserve={handleBookAction} onCancelReserve={handleCancelReserve}/>
{/if}